...
{
    ros::init(argc, argv, "stablized_pose_tuner");
    ...
    //creating a shadow pose here and its gain /rn
    double angle_shadow;
    vector<double> my_pose_shadow;
    double gain=1; //tune the gain to make it more stable /rn
    while(!called && ros::ok()) {
    ...
    while (ros::ok()) {
        ...
        my_pose_shadow.resize(current_joint_states.name.size(), 0.0);


    //initialising pose and pose shadow /rn
        for (int i = 0; i < my_pose.size(); ++i) {
            my_pose = current_joint_states.position;
        }
        for (int i = 0; i < my_pose_shadow.size(); ++i) {
            my_pose_shadow = current_joint_states.position;
        }
        ...
        //loading shadow message  /rn
        angle_shadow = angle * gain;
        my_pose_shadow[joint] += angle_shadow;


 	ROS_INFO("my_pose after input %f",my_pose[joint]);
        //first, send the desired trajectory command  /rn
        sendArmCommand(my_pose);
        //you may aggressively set a large value for observation  /rn
        //this number cannot be small otherwise the effect will not show
        ros::Duration(3).sleep();
        //after that, send the shadow to stablize the robot  /rn
        sendArmCommand(my_pose_shadow);
